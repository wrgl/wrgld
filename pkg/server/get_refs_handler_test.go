...
func (s *testSuite) TestGetRefsHandler(t *testing.T) {
	...
	refs, err = cli.GetRefs([]string{"heads/"}, nil)
	require.NoError(t, err)
	assert.Equal(t, map[string][]byte{
		"heads/" + head: sum1,
	}, refs)

	refs, err = cli.GetRefs(nil, []string{"heads/"})
	require.NoError(t, err)
	assert.Equal(t, map[string][]byte{
		"tags/" + tag: sum2,
	}, refs)
	...
	req := m.Capture(t, func(header http.Header) {
		...
		refs, err := cli.GetRefs(nil, nil, apiclient.WithRequestHeader(header))
		require.NoError(t, err)

...
func (s *testSuite) TestAuthenticate(t *testing.T) {
	...
	assert.Error(t, err)
	_, err = cli.GetRefs(nil, nil, apiclient.WithRequestAuthorization(tok))
	assert.Error(t, err)
	...
	assert.Error(t, err)
	_, err = cli.PostReceivePack(map[string]*payload.Update{"main": {OldSum: payload.BytesToHex(sum2)}}, nil, apiclient.WithRequestAuthorization(tok))
	assert.Error(t, err)
	...
	refs, err := cli.GetRefs(nil, nil, apiclient.WithRequestAuthorization(readTok))
	require.NoError(t, err)
	...
	resp, err = cli.PostReceivePack(map[string]*payload.Update{"main": {OldSum: payload.BytesToHex(sum2)}}, nil, apiclient.WithRequestAuthorization(writeTok))
	require.NoError(t, err)
	assert.Equal(t, http.StatusOK, resp.StatusCode)

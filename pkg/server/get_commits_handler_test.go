...
func (s *testSuite) TestGetCommits(t *testing.T) {
	...
	assertCommitTreeEqual(t, server.CommitPayload(db, com7), &gcr.Root)

	gcr, err = cli.GetCommits("heads/main", 1)
	require.NoError(t, err)
	assert.Equal(t, sum7, (*gcr.Sum)[:])
	assertCommitTreeEqual(t,
		setParentCommits(server.CommitPayload(db, com7), map[string]*payload.Commit{
			hex.EncodeToString(sum5): server.CommitPayload(db, com5),
			hex.EncodeToString(sum6): server.CommitPayload(db, com6),
		}),
		&gcr.Root,
	)
	...
	assertCommitTreeEqual(t,
		setParentCommits(server.CommitPayload(db, com4), map[string]*payload.Commit{
			hex.EncodeToString(sum2): setParentCommits(server.CommitPayload(db, com2), map[string]*payload.Commit{
				hex.EncodeToString(sum1): server.CommitPayload(db, com1),
			}),
			hex.EncodeToString(sum3): server.CommitPayload(db, com3),
		}),
		&gcr.Root,
	)
